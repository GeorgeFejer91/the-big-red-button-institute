...
- The native Quest study button GLB is included as an alternative model profile
  without replacing the classic Unity model.
...
Build the same APK with the native Quest study button model through:

```powershell
Tools\Build-UnityStudyApk.ps1 -ButtonModelProfile NativeQuestStudy
```
